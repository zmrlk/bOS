...
- **Read:** `state/context-bus.jsonl` (the last unexpired critical entry is injected at SessionStart; `tail -20` when you need more). `state/context-bus.md` is a stub.
- **Write:** only via `bash scripts/context-bus-append.sh` — never append to the jsonl by hand.
...
- **Write:** projects.md (tech stack, estimates, security status); context-bus.jsonl via the append helper only
